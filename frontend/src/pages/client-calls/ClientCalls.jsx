...
import { FiPhoneCall, FiPlus, FiClock, FiCalendar, FiSave, FiEdit2, FiTrash2, FiPlay, FiSquare } from "react-icons/fi";
...
const ClientCalls = ({ isEmbedded = false }) => {
  ...
  const stopTimer = () => {
    ...
    if (mins > 0 || hrs > 0) durationStr += `${mins} min${mins !== 1 ? 's' : ''} `;
    if (secs > 0 || (hrs === 0 && mins === 0)) durationStr += `${secs} sec${secs !== 1 ? 's' : ''}`;
      ...
      {!isEmbedded && (
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Client Calls</h1>
          </div>
          {!showAddForm && (
            <button 
              onClick={() => {
        ...
        <div className="bg-white dark:bg-[#0B1120] rounded-xl border border-indigo-100 dark:border-indigo-900/30 p-6 mb-8 shadow-sm transition-all duration-300">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold text-slate-800 dark:text-white flex items-center">
              {editId ? <FiEdit2 className="mr-2 text-indigo-500" /> : <FiPlus className="mr-2 text-indigo-500" />}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg p-3 flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="text-xs text-slate-500 dark:text-slate-400">
                        {formData.startTime && !isTimerRunning && formData.endTime ? (
                          <span>Call finished ({formatTime12hr(formData.startTime)} - {formatTime12hr(formData.endTime)})</span>
                        ) : isTimerRunning ? (
                          <span className="flex items-center text-emerald-500"><span className="animate-pulse mr-1 h-2 w-2 bg-emerald-500 rounded-full"></span> Call in progress...</span>
                        ) : (
                    ...
                    <div>
                      {!isTimerRunning && elapsedSeconds === 0 ? (
                        ...
                        <button
                          ...
                          onClick={() => {
                            setElapsedSeconds(0);
                            setFormData(prev => ({ ...prev, startTime: "", endTime: "", duration: "" }));
                          }}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Discussion Points</label>
              <textarea
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800">
              <div className="text-sm text-slate-500 dark:text-slate-400">
                Duration: <span className="font-semibold text-slate-700 dark:text-slate-300">{formData.duration || '--'}</span>
              </div>
              <div className="flex space-x-3">
                ...
                >
                  <FiSave className="mr-2" /> {submitting ? 'Saving...' : editId ? 'Update Record' : 'Save Record'}
                </button>
              </div>
            </div>
          </form>
      ...
      <div className="bg-white dark:bg-[#0B1120] rounded-xl border border-slate-200 dark:border-slate-800/60 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200">No Calls Logged</h3>
            <p className="text-slate-500 dark:text-slate-400 mt-1 mb-4">You haven't recorded any client calls yet.</p>
            <button 
              onClick={() => {
                setEditId(null);
                setTimeMode("timer");
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60">
                {calls.map((call) => (
                  <tr key={call._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors group">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 dark:text-slate-400">
                      <div className="flex items-center">
                        ...
                        {formatTime12hr(call.startTime)} - {formatTime12hr(call.endTime)}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 dark:text-slate-400">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400 max-w-md">
                      <div className="line-clamp-2" title={call.discussionPoints}>
